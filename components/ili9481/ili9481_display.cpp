...
static const char *const TAG = "ili9481";

void ILI9481Display::setup_pins_() {
...
void ILI9481Display::dump_config() {
  LOG_DISPLAY("", "ili9481", this);
...
float ILI9481Display::get_setup_priority() const { return setup_priority::PROCESSOR; }
void ILI9481Display::command(uint8_t value) {
  this->start_command_();
  this->write_byte(value);
  this->end_command_();
}

void ILI9481Display::reset_() {
  if (this->reset_pin_ != nullptr) {
...
void ILI9481Display::data(uint8_t value) {
  this->start_data_();
  this->write_byte(value);
  this->end_data_();
}

void ILI9481Display::send_command(uint8_t command_byte, const uint8_t *data_bytes, uint8_t num_data_bytes) {
...
uint8_t ILI9481Display::read_command(uint8_t command_byte, uint8_t index) {
...
void ILI9481Display::fill_internal_(Color color) {
...
void HOT ILI9481Display::draw_absolute_pixel_internal(int x, int y, Color color) {
...
uint32_t ILI9481Display::get_buffer_length_() { return this->get_width_internal() * this->get_height_internal(); }

void ILI9481Display::start_command_() {
  this->dc_pin_->digital_write(false);
  this->enable();
}

void ILI9481Display::end_command_() { this->disable(); }
void ILI9481Display::start_data_() {
  this->dc_pin_->digital_write(true);
  this->enable();
}
void ILI9481Display::end_data_() { this->disable(); }

void ILI9481Display::init_lcd_(const uint8_t *init_cmd) {
...
void ILI9481Display::set_addr_window_(uint16_t x1, uint16_t y1, uint16_t w, uint16_t h) {
  uint16_t x2 = (x1 + w - 1), y2 = (y1 + h - 1);
  this->command(ILI9481_CASET);  // Column address set
  this->start_data_();
  ...
  this->write_byte(x2);
  this->end_data_();
  this->command(ILI9481_PASET);  // Row address set
  this->start_data_();
  ...
  this->write_byte(y2);
  this->end_data_();
  this->command(ILI9481_RAMWR);  // Write to RAM
}

void ILI9481Display::invert_display_(bool invert) { this->command(invert ? ILI9481_INVON : ILI9481_INVOFF); }

int ILI9481Display::get_width_internal() { return this->width_; }
int ILI9481Display::get_height_internal() { return this->height_; }

uint32_t ILI9481Display::buffer_to_transfer_(uint32_t pos, uint32_t sz) {
...
//   M5Stack display
void ILI9481M5Stack::initialize() {
